...
const AppSidebar: React.FC = () => {
        ...
        <li key={nav.name} className="relative">
          {nav.subItems ? (
               ...
               className={`menu-item group ${
                ...
                openSubmenu?.type === menuType && openSubmenu?.index === index && (isExpanded || isMobileOpen) // intentionally NOT using hover for active
                  ? "menu-item-active"

                  : "menu-item-inactive"

                             } cursor-pointer transition-all duration-200 hover:bg-gray-50 dark:hover:bg-gray-800 ${
               ...
               title={!isExpanded && !isHoverExpanded ? nav.name : undefined}
            >
              ...
              {(isExpanded || isMobileOpen || isHoverExpanded) && (
                <span className="menu-item-text">{nav.name}</span>
              )}

              {/* Parent active vertical marker - shows if any child route is active */}
              {nav.subItems && nav.subItems.some((s) => isActive(s.path)) && (
                <span className="absolute left-0 top-1/2 -translate-y-1/2 w-[2px] h-6 bg-brand-500 rounded" />
              )}
            ...
            nav.path && (
                 ...
                 className={`menu-item group transition-all duration-200 hover:bg-gray-50 dark:hover:bg-gray-800 ${
                   isActive(nav.path) && (isExpanded || isMobileOpen) ? "menu-item-active" : "menu-item-inactive" // hover-expanded should not affect active
                 }`}
          ...
          {nav.subItems && (isExpanded || isMobileOpen || isHoverExpanded) && (
            <div
              ...
              style={{
                ...
                height: (openSubmenu?.type === menuType && openSubmenu?.index === index)
                  ? (subMenuRefs.current[`${menuType}-${index}`]?.scrollHeight || subMenuHeight[`${menuType}-${index}`] || 0)
                    : 0,
  ...
  return (
    ...
    <aside
      ...
      className={`fixed mt-16 flex flex-col lg:mt-0 top-0 px-5 left-0 bg-white dark:bg-gray-900 dark:border-gray-800 text-gray-900 h-screen transition-all duration-100 ease-out z-[100000] border-r border-gray-200 
      ...
      onMouseEnter={() => {
        if (!isExpanded) {
          setIsHoverExpanded(true);
          const header = document.querySelector('.header-inner') as HTMLElement | null;
          if (header) {
            header.style.maxWidth = 'calc(100vw - 290px)';
          }
        }
      }}
      onMouseLeave={() => {
        if (!isExpanded) {
          ...
          const header = document.querySelector('.header-inner') as HTMLElement | null;
          if (header) {
            header.style.maxWidth = 'calc(100vw - 0px)';
          }
        }
      }}
    >
      ...
      <div
        ...
        className={`py-8 flex items-center ${
          ...
          !isExpanded && !isHoverExpanded ? "lg:justify-center" : "justify-start"

        } gap-2 sticky top-0 bg-white dark:bg-gray-900 z-[99999]`}

      >

        <button

          className="items-center justify-center w-10 h-10 text-gray-500 border-gray-200 rounded-lg z-99999 dark:border-gray-800 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all duration-200 active:scale-95 hover:scale-105"
          onClick={(e) => {

            e.preventDefault();

            // Ensure hover-expanded does not keep the sidebar visually open after toggle
            setIsHoverExpanded(false);
            // If we're about to collapse, close any open submenu
            if (isExpanded) {
              setOpenSubmenu(null);
            }
            toggleSidebar();
      ...
      {user && accessToken && (
        ...
        <div ref={scrollContainerRef} className="flex flex-col overflow-y-auto duration-150 ease-out custom-scrollbar">
